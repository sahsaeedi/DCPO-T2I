...
DATASET_NAME_MAPPING = {
    ...
    "pickapic": ("jpg_0", "jpg_1", "label_0", "caption"),
}
...
def main():
    ...
    caption_0 = args.caption_0
    caption_1 = args.caption_1

    def tokenize_captions(examples, is_train=True, flag=True):
        if caption_0 and caption_1 and flag:
            captions_0 = []
            captions_1 = []
            for cap_0, cap_1 in zip(examples[caption_0], examples[caption_1]):
                captions_0.append(cap_0)
                captions_1.append(cap_1)

            inputs_0 = tokenizer(
                captions_0, max_length=tokenizer.model_max_length, padding="max_length", truncation=True, return_tensors="pt"
            )

            inputs_1 = tokenizer(
                captions_1, max_length=tokenizer.model_max_length, padding="max_length", truncation=True, return_tensors="pt"
            )

            return inputs_0.input_ids, inputs_1.input_ids
        else:
            raise ValueError(
                        f"Caption columns have problems."
                    )
    ...
    if args.train_method == 'dpo':
        ...
        def preprocess_train(examples):
            ...
            tokens_0, tokens_1 = tokenize_captions(examples)
            all_token_prompts = [tokens_0, tokens_1]

            im_tup_iterator = zip(*all_pixel_values)
            token_tup_iterator = zip(*all_token_prompts)

            combined_pixel_values = []
            combined_token_values = []
            for im_tup, token_tup, label_0 in zip(im_tup_iterator, token_tup_iterator, examples['label_0']):
                if label_0==0 and (not args.choice_model): 
                    im_tup = im_tup[::-1]
                    token_tup = token_tup[::-1]
                combined_im = torch.cat(im_tup, dim=0) 
                combined_token = torch.cat(token_tup, dim=0)
                combined_pixel_values.append(combined_im)
                combined_token_values.append(combined_token)
                
            examples["pixel_values"] = combined_pixel_values
            examples["input_ids_temp"] = combined_token_values
            return examples
        ...
        def collate_fn(examples):
            ...
            if args.sdxl:
                ...
                return_d["input_ids_temp"] = torch.stack([example["input_ids_temp"] for example in examples])
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(unet):
                ...
                if args.sdxl:
                    ...
                    encoder_h_s_1 = torch.cat(batch["input_ids_temp"].chunk(2, dim=1))
                    encoder_h_s_1 = text_encoder(encoder_h_s_1)[0]

                    if args.train_method == 'dpo':
                        
                        encoder_hidden_states = encoder_h_s_1
                ...
                avg_loss = accelerator.gather(loss.repeat(args.train_batch_size)).mean() # loss calculation
